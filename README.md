...
ユーザーがプロンプトをAPIにてエージェントに送信、指示通りのプログラムをAWSで実行
...
### 📊 データ駆動型分析
ユーザーはプロンプトを送信するだけで:
- 複雑なデータ分析とレポートの自動生成（成果物はAWSのストレージ（**s3**）に格納）
- 機械学習モデルの構築

### 🧠 スキーマ認識エージェント
エージェントは**データベーススキーマを理解**しており、以下が可能:
- 適切なSQLクエリの生成
- データベースにあるテーブルの分析コードを作成
