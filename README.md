...
- Query and reason over **Amazon Redshift** tables using Claude sonnet 4.0 from **Amazon Bedrock** and generate a python code.
- Run the python code generated by the agent on **ECS Fargate**.
- Any outputs produced by the agent are saved in **s3** (see `/results`)

The agent understands the database schema learned from **DSPy**:
